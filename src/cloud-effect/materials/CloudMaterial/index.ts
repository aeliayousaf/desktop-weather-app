    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        float getCloudDensity(vec3 p) {
          float scale = 2.0;
          vec3 coord = p * scale;
          coord.x += uTime * (0.06 + uIntensity * 0.14);
          coord = mod(coord, 1.0);
          ...
          float densityCutoff = mix(0.4, 0.02, uIntensity);
          float cloudDensity = saturate(perlinWorley - (1.0 - dimensionalProfile) - densityCutoff);
