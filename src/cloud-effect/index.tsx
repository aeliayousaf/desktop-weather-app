...
export class CloudsRenderer {
  ...
  resize(size: Size) {
    const downSampleFactor = 0.5;
    const width = Math.max(1, Math.floor(size.width * downSampleFactor));
    const height = Math.max(1, Math.floor(size.height * downSampleFactor));
    this.textureScene.setSize(width, height);
    this.textureCloud.setSize(width, height);
  }

  setIntensity(intensity: number) {
    const t = Math.max(0, Math.min(100, intensity)) / 100;
    this.cloudMaterial.uniforms.uIntensity.value = t;
    this.renderMaterial.uniforms.uIntensity.value = t;
  }
